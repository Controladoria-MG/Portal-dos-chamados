...
atualizar_base.py
─────────────────
Lê os relatórios de chamados (Geral, GC, Controladoria, Paralegal e
Cadastro de Produtos) da pasta Att Base, substitui usuários AD das
...
Estrutura esperada:
  ...
  │       ├── Chamados - Cadastro de Produtos - *.xlsx  (opcional)
...
def main():
    ...
    # Arquivos específicos: GC, Controladoria, Paralegal e Cadastro de Produtos (pega o mais recente de cada)
    ...
    prod_path = find_file(ATT_DIR, "*Chamados - Cadastro de Produtos*.xlsx", required=False)
    ...
    prod_name = prod_path.name if prod_path else None

    # Chamados gerais: todos com "Chamados" no nome, exceto GC/Controladoria/Paralegal/Cadastro de Produtos
    chamados_paths = [
        p for p in find_all(ATT_DIR, "*Chamados*.xlsx")
        if p.name != gc_name and p.name != ctr_name and p.name != pl_name and p.name != prod_name
    ]
    ...
    print(f"  Cad. Produtos: {prod_name or 'nao encontrado'}")
    ...
    if prod_path: fontes.append(("Cadastro de Produtos", prod_path))
