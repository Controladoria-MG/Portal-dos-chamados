...
/* Remove acentos e caixa para comparação de busca ("gerencia" ~ "Gerência") */
function normalizeSearch(str) {
  return String(str || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
}

/* Acima de quantas opções o campo de busca do dropdown passa a valer a pena
   (listas curtas, ex.: Status/Vencimento, não precisam de busca). */
const CAT_SEARCH_MIN_OPTIONS = 8;
...
/* Cards totalizadores do departamento (Total / Em Atraso / Vencendo Hoje / Em Aberto) —
   mesmo cálculo dos cards da tela de filial, aplicado aos chamados visíveis
   (após filtros/busca) do departamento aberto. */
...
function updateTicketStats(tickets) {
  ...
  if (!$deptCardTotal) return;
  const stats = contarStatusVencimento(tickets);
  $deptCardTotal.textContent  = stats.total.toLocaleString('pt-BR');
  $deptCardAtraso.textContent = stats.atraso.toLocaleString('pt-BR');
  $deptCardHoje.textContent   = stats.hoje.toLocaleString('pt-BR');
  $deptCardAberto.textContent = stats.aberto.toLocaleString('pt-BR');
}
...
const $deptCardTotal    = document.getElementById('dept-card-total');
const $deptCardAtraso   = document.getElementById('dept-card-atraso');
const $deptCardHoje     = document.getElementById('dept-card-hoje');
const $deptCardAberto   = document.getElementById('dept-card-aberto');
...
/* ─── HOME VIEW (FILIAIS) ────────────────────────────────────────────
   ...
   + o card do Cadastro de Produtos, que não pertence a nenhuma filial e
   fica direto nesta tela. Departamentos de cada filial ficam na tela
   seguinte (view-filial); Cadastro de Produtos vai direto pro detalhe
   (view-dept), já que não tem sub-departamentos. ── */
function renderHome() {
  $loading.style.display = 'none';
  ...
  cardProdutos.className = 'dept-card';
  cardProdutos.innerHTML = `
    <div class="card-name">Cadastro de Produtos</div>
    <div class="card-hint">Clique para ver os detalhes</div>`;
  cardProdutos.addEventListener('click', () => {
    currentFilial = null; // não pertence a nenhuma filial — volta pra home, não pra tela de filial
    openDept('CADASTRO DE PRODUTOS');
  });
  $deptGrid.appendChild(cardProdutos);
...
function renderFilialDepts(filialNome) {
  ...
  for (const row of allData) {
    ...
    if (dept === 'CADASTRO DE PRODUTOS') continue; // agora é card próprio na tela inicial, não pertence a nenhuma filial
  ...
  // Garante que a Gerência de Contas e o Paralegal (só em SP) da filial
  // aparecem mesmo sem pendentes no momento.
...
function buildFilterDropdowns() {
  // Busca numa linha em cima; filtros + "Limpar filtros" na fileira embaixo.
  ...
  topRow.className = 'filters-top-row';
  controls.appendChild(topRow);
  topRow.appendChild(searchInput);

  // Botão "Limpar filtros" — só some quando algum filtro está ativo, pra
  // não precisar abrir cada dropdown e clicar em "Limpar" um por um.
  const clearAllBtn = document.createElement('button');
  clearAllBtn.type      = 'button';
  clearAllBtn.id        = 'btn-clear-all-filters';
  clearAllBtn.className = 'btn-clear-filters';
  clearAllBtn.textContent = '✕ Limpar filtros';
  clearAllBtn.addEventListener('click', () => {
    FILTER_DEFS.forEach(def => {
      document.querySelectorAll(`#${def.key}-list input[type=checkbox]`).forEach(cb => cb.checked = false);
  ...
  FILTER_DEFS.forEach(def => {
    ...
    dd.innerHTML = `
      <div class="cat-dropdown-search">
        <input type="text" id="${def.key}-search" class="cat-search" placeholder="Buscar..." autocomplete="off">
      </div>
    ...
    const searchWrap = dd.querySelector('.cat-dropdown-search');
    const searchBox  = dd.querySelector(`#${def.key}-search`);
    searchBox.addEventListener('input', () => filterCatOptions(def.key, searchBox.value));
  ...
  filtersRow.appendChild(clearAllBtn);
...
function populateFilterDropdowns(rows) {
  FILTER_DEFS.forEach(def => {
    ...
    for (const val of values) {
      ...
      li.className = 'cat-option-item';
      li.dataset.label = isVazio ? 'vazio' : val;
      li.innerHTML = `
    ...
    // Busca dentro do dropdown só compensa em listas longas — em listas
    // curtas (ex.: Status/Vencimento) ela é só ruído a mais na tela.
    const $searchWrap = document.getElementById(`${def.key}-dropdown`)?.querySelector('.cat-dropdown-search');
    const $searchBox   = document.getElementById(`${def.key}-search`);
    if ($searchWrap && $searchBox) {
      const temBusca = values.length > CAT_SEARCH_MIN_OPTIONS;
      $searchWrap.hidden = !temBusca;
      $searchBox.value   = '';
      filterCatOptions(def.key, '');
    }
...
/* ─── FILTROS — BUSCA DENTRO DO DROPDOWN ─────────────────────────── */
function filterCatOptions(key, query) {
  const $list = document.getElementById(`${key}-list`);
  if (!$list) return;
  const q = normalizeSearch(query);
  let anyVisible = false;
  $list.querySelectorAll('li.cat-option-item').forEach(li => {
    const match = !q || normalizeSearch(li.dataset.label).includes(q);
    li.hidden = !match;
    if (match) anyVisible = true;
  });

  let $empty = $list.querySelector('.cat-option-no-match');
  if (!anyVisible) {
    if (!$empty) {
      $empty = document.createElement('li');
      $empty.className = 'cat-option-no-match';
...
function renderDeptRows(allDeptRows) {
  ...
  thead.innerHTML = '<th>ID Cliente</th><th>Cliente</th><th>Categoria</th><th class="date-cell">Data Cadastro</th><th class="previsao-cell">Previsão Atend.</th><th class="prazo-cell">Prazo Vencimento</th>';
...
function agruparPorCliente(tickets) {
  ...
  Object.values(clientMap).forEach(c => {
    ...
    c.categorias = [...new Set(c.rows.map(r => String(col(r,'Categoria','categoria')||'').trim()).filter(Boolean))];
  });
...
function renderClientTable(clients) {
  ...
  if (!clients.length) {
    $clientBody.innerHTML = `<tr><td colspan="6" class="empty">Nenhum registro encontrado.</td></tr>`;
    return;
  }
  ...
  clients.forEach((c, i) => {
    ...
    tr.innerHTML = `
      ...
      <td class="categoria-cell">${escHtml(c.categorias.join(', '))||'—'}</td>
...
document.getElementById('btn-back').addEventListener('click', () => {
  ...
  showView(currentFilial ? 'filial' : 'home');
});
...
function showView(name) {
  ...
  const partesPorView = {
    ...
    'dept':              currentFilial
      ? [{ label: currentFilial, onClick: () => showView('filial') }, { label: currentDept }]
      : [{ label: currentDept }], // Cadastro de Produtos: sem filial, aberto direto da home
    'relatorios-filial': [{ label: currentRelFilial }],
    'relatorios':        currentRelFilial
      ? [{ label: currentRelFilial, onClick: () => showView('relatorios-filial') },
         { label: currentRelDept ? currentRelDept.nome : '' }]
      : [{ label: currentRelDept ? currentRelDept.nome : '' }], // Cadastro de Produtos: sem filial
  };
...
document.getElementById('btn-back-relatorios').addEventListener('click', () => {
  showView(currentRelFilial ? 'relatorios-filial' : 'relatorios-home');
});
...
function renderRelatoriosFiliais() {
  ...
  // Cadastro de Produtos não pertence a nenhuma filial — card próprio aqui,
  // direto pro relatório do departamento (mesma lógica da tela principal).
  if (deptos.some(d => d.nome === 'CADASTRO DE PRODUTOS')) {
    const cardProdutos = document.createElement('div');
    cardProdutos.className = 'dept-card';
    cardProdutos.innerHTML = `
      <div class="card-name">Cadastro de Produtos</div>
      <div class="card-hint">Clique para ver o relatório</div>`;
    cardProdutos.addEventListener('click', () => {
      currentRelFilial = null;
      openRelatorioDept('CADASTRO DE PRODUTOS');
    });
    $grid.appendChild(cardProdutos);
  }
}
...
function renderRelatoriosFilialDepts(filialNome) {
  const deptos = ((detalheMensal && detalheMensal.departamentos) || [])
    .filter(d => d.nome !== 'CADASTRO DE PRODUTOS' && filialDoDept(d.nome) === filialNome);
...
/* ─── PLANILHA DE COBRANÇA ───────────────────────────────────────────
   ...
   Departamento Responsavel / Data Cadastro / Situacao / Prazo Vencimento /
...
const PLANILHA_COBRANCA_COLS_DATA = new Set(['Data Cadastro', 'Prazo Vencimento', 'Data Previsao Atendimento']);
...
function linhaPlanilhaCobranca(r) {
  return {
    ...
    'Prazo Vencimento':            dateParaSerialExcel(parsePrazoDate(prazoVal(r))),
...
const PLANILHA_COBRANCA_COLS = [
  ...
  { header: 'Prazo Vencimento',          wch: 15 },
